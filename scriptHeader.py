...
from utils import ThrowError, getCurrentMeta
import json
...
def aliasSetExpr(regexp):
    global ALIAS_KEY_WORD, ALIAS_OTHER_SYMBOLES, ALIAS_SYNTAX_LENGTH
    global ALIAS_NAME_SYMBOLE_POS, ALIAS_TYPE_KEYWORD_POS, ALIAS_VALUE_SYMBOLE_POS
    regexp = regexp.split()
    ...
    for i in range(1, len(regexp)):
        if regexp[i] == "%n":
            ALIAS_NAME_SYMBOLE_POS = i
        elif regexp[i] == "%t":
            ALIAS_TYPE_KEYWORD_POS = i
        elif regexp[i] == "%v":
            ALIAS_VALUE_SYMBOLE_POS = i
        else:
            ALIAS_OTHER_SYMBOLES.append((i, regexp[i]))
    ...
    if symboles[i].sym == ALIAS_KEY_WORD:
        ...
        for s in ALIAS_OTHER_SYMBOLES:
            if symboles[i + s[0]].sym != s[1]:
                if ALIAS_DEBUG:
                    print(s[1], symboles[i + s[0]].sym)
                ThrowError(
                    f"expected symbol '{s[1]}'",
                    symboles[i + s[0]].line,
                    symboles[i + s[0]].column,
                    len(symboles[i + s[0]].sym),
                )
        ...
        for t in ALIAS_TYPES:
            if symboles[i + ALIAS_TYPE_KEYWORD_POS].sym in t[0]:
                if not t[1](aliasVal.sym):
                    ThrowError(
                        f"expected {t[3]} name for alias",
                        aliasVal.line,
                        aliasVal.column,
                        len(symboles[i + ALIAS_VALUE_SYMBOLE_POS].sym),
                    )
                if t[2](aliasVal.sym, aliasName.sym) < 0:
                    ThrowError(
                        f"name {aliasName.sym} is already used",
                        aliasName.line,
                        aliasName.column,
                        len(symboles[i + ALIAS_NAME_SYMBOLE_POS].sym),
                    )
                if ALIAS_DEBUG:
                    print(
                        f"\033[93m<DEBUG>\033[0m [ALIASES] Added {t[3]}"
                        + " alias {aliasName.sym} for value {aliasVal.sym}"
                    )
                break
        else:
            ThrowError(
                f"incorrect alias type {symboles[i+ALIAS_TYPE_KEYWORD_POS].sym}",
                symboles[i + ALIAS_TYPE_KEYWORD_POS].line,
                symboles[i + ALIAS_TYPE_KEYWORD_POS].column,
                len(symboles[i + ALIAS_TYPE_KEYWORD_POS].sym),
            )
...
def checkMetaStruct(symboles, i, meta):
    ...
    if metaVal == symboles[i + 2].sym:
        ThrowError(
            f"expected string for field '{metaName}'",
            symboles[i + 2].line,
            symboles[i + 2].column,
            len(symboles[i + 2].sym),
        )
...
def writeHeaderMetaFile(h, f, f2, f3):
    ...
    for m in currentMeta:
        if (
            m["file_name"] == headerMeta["file_name"]
            or m["script_name"] == headerMeta["script_name"]
            or m["origin_file"] == headerMeta["origin_file"]
        ):
            removeVals.append(m)
    for v in removeVals:
...
class ScriptHeader:
    ...
    def __init__(self, text):
        ...
        for i in range(len(symboles)):
            ...
            if ALIAS_ENABLE:
                skip = checkAliasStruct(symboles, i)
            if skip > 0:
                continue
            ...
            if skip > 0:
                continue
            ThrowError(
                f"unknown symbol {symboles[i].sym}",
                symboles[i].line,
                symboles[i].column,
                len(symboles[i].sym),
            )
