...
def strIsInt(s):
    ...
    if s[0] == '-' or s[0] == '+': return strIsInt(s[1:])
...
def HexToIntDigit(c):
    try:
        ...
        if c.lower() == 'a': return 10
        if c.lower() == 'b': return 11
        if c.lower() == 'c': return 12
        if c.lower() == 'd': return 13
        if c.lower() == 'e': return 14
        if c.lower() == 'f': return 15
...
def ThrowError(error, l, c, length = 1):
    ...
    for i, ch in enumerate(fileContent.split("\n")[l]):
        if i == c: codeLine += COL.setFG(1) + COL.BOLD
        if i == c+length: codeLine += COL.RESET
        codeLine += ch
    firstLine = COL.BOLD + filename+":"+str(l+1)+":"+str(c)+": " + COL.setFG(1) + "error: " + COL.RESET + error
    secondLine = nbspaces * " " + str(l+1) + " | " + codeLine
    thirdLine = "      | " + c * " " + COL.setFG(1) + COL.BOLD + "^" + max(0,length-1) * "~" + COL.RESET
    ShowLocation()
    print(firstLine)
    print(secondLine)
    print(thirdLine)
